...
experiment_dir = "exp_ipopt_$(d)_$(n)_" * timestamp
...
function train_and_eval_model(K, eps_CL, eps_E, tol)
    ...
    logger = SimpleLogger(open("$(experiment_dir)/output_$(iteration_counter).txt", "w+"))
    with_logger(logger) do
        start_time, start_optimization, end_time,
        opt_times = eSPA.fit!(model, X_train, y_train)
    end

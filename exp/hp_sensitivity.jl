...
header = DataFrame(K = Int[], eps_CL = Float64[], eps_E = Float64[], tol = Float64[],
                   time = Float64[], acc = Float64[])
...
function train_and_eval_model(K, eps_CL, eps_E, tol)
    ...
    #Track data
    result_row = DataFrame(K = [K], eps_CL = [eps_CL], eps_E = [eps_E], tol = [tol],
                           time = [complete_time], acc = [acc])

...
function eSPAhybrid(K::Int, eps_CL::Float64, eps_E::Float64, tol::Float64,max_iter::Int,num_fuzzy_steps::Int)
    if eps_CL < 0.0
    ...
    T = 0
    M = 0
    return eSPAhybrid(K, eps_CL, eps_E, tol, max_iter, num_fuzzy_steps, gamma, W, S, lambda, Pi, D, T, M)
end
...
function fit!(model::eSPAhybrid, X::AbstractMatrix, y::AbstractVector)
    ...
    end_time = time_ns()
    return start_time, start_optimization, start_fuzzy, end_time, opt_times_discr, opt_times_fuzzy
end

...
"""
mutable struct eSPAhybrid_test
    # Hyperparameters
    K::Int
    eps_CL::Float64
    eps_E::Float64
...
function eSPAhybrid_test(K::Int, eps_CL::Float64, eps_E::Float64, tol::Float64,max_iter::Int,num_fuzzy_steps::Int)
    if eps_CL < 0.0
    ...
    T = 0
    M = 0
    return eSPAhybrid_test(K, eps_CL, eps_E, tol, max_iter, num_fuzzy_steps, gamma, W, S, lambda, Pi, D, T, M)
end
...
"""
function fit!(model::eSPAhybrid_test, X::AbstractMatrix, y::AbstractVector)
    start_time = time_ns()
    ...
    opt_times_discr = DataFrame(i=Int[],no_empty_cluster=Int[],sstep=Int[],lambdastep=Int[],gammastep=Int[],wstep=Int[],loss=Int[])
    ...
    plus_model = eSPAplus(model.K, model.eps_CL, model.eps_E, model.tol, model.max_iter, copy(model.gamma), copy(model.W), copy(model.S), copy(model.lambda), copy(model.Pi), model.D, model.T, model.M)
    opt_times_fuzzy = DataFrame(i=Int[],sstep=Int[],lambdastep=Int[],gammastep=Int[],wstep=Int[],loss=Int[])
    start_fuzzy = time_ns()
    ...
    for fuzzy_step in 1:model.num_fuzzy_steps
        ...
        timing_results = (;i = fuzzy_step,sstep=time_2-time_1,lambdastep=time_3-time_2,gammastep=time_4-time_3,wstep=time_5-time_4,loss=time_6-time_5)
        push!(opt_times_fuzzy,timing_results)
    end
    end_time = time_ns()


    return start_time, start_optimization, start_fuzzy, end_time, opt_times_discr, opt_times_fuzzy, plus_model
end
...
- `X::AbstractMatrix`: Data matrix which includes the features. Rows contain the features, Columns the data point.
"""
function predict(model::eSPAhybrid_test, X::AbstractMatrix)
    D, T = size(X)
    if model.D != D

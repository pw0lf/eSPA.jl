...
function eSPAhybrid_test(K::Int, eps_CL::Float64, eps_E::Float64, tol::Float64,
                         max_iter::Int, num_fuzzy_steps::Int)
    if eps_CL < 0.0
    ...
    T = 0
    M = 0
    return eSPAhybrid_test(K, eps_CL, eps_E, tol, max_iter, num_fuzzy_steps, gamma, W, S,
                           lambda, Pi, D, T, M)
end
...
function fit!(model::eSPAhybrid_test, X::AbstractMatrix, y::AbstractVector)
    ...
    plus_model = eSPAplus(model.K, model.eps_CL, model.eps_E, model.tol, model.max_iter,
                          copy(model.gamma), copy(model.W), copy(model.S),
                          copy(model.lambda), copy(model.Pi), model.D, model.T, model.M)

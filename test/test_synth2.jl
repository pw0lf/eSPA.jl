...
scatter!(model.S[1,:],model.S[2,:],label="",marker = :diamond,markersize=4,color= :red)

function plot_it(i,x,y)
    cls = clusters[i]
    scatter(X_test[x,(y_test .== 1)],X_test[y,(y_test .== 1)],zcolor=y_pred[(y_test .== 1)],label="",alpha=0.5,marker = :circle)
    scatter!(X_test[x,(y_test .== 2)],X_test[y,(y_test .== 2)],zcolor=y_pred[(y_test .== 2)],label="",alpha=0.5, marker = :square) 
    scatter!(cls[x,:],cls[y,:],label="",marker = :diamond,markersize=4,color= :red)
end